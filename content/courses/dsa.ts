...
ADT (What operations)
       ↓
Data Structure (How it is implemented)
       ↓
Algorithms (Steps to perform operations)
\`\`\`
...
Each element connects to exactly ONE next element
\`\`\`
...
\`\`\`
// Example: Calculate sum of n numberssum = 0;
// Step 1  → O(1) — executes oncefor i = 1 to n
// Step 2  → O(n) — executes n times   sum = sum + i;
// Step 3  → O(n) — executes n timesreturn sum;
// Step 4  → O(1) — executes once// Total: O(1) + O(n) + O(n) + O(1) = O(n) — Linear Time
\`\`\`
...
int getFirst(int arr[]) {
  ...
  return arr[0];  // just 1 operation regardless of array size
}
...
// Example 3 — O(n²) quadratic (nested loops)
void printPairs(int arr[], int n) {

  for(int i=0; i<n; i++)       // outer loop: n times
for(int j=0; j<n; j++)     // inner loop: n times per outer
printf('%d %d\\n', arr[i], arr[j]);
}
...
int low=0, high=n-1;
  while(low <= high) {
    ...
    if(arr[mid]<key) low=mid+1;  // HALVES the search space
else high=mid-1;             // each iteration halves → O(log n)  
}
...
\`\`\`
Variable:    int a = 5;  Memory:  Address | Value
--------|-------
1000    |  5
← variable a stored here
1004    |  ...   ← next memory location
Pointer:  int *b = &a;  (b stores address 1000)  Memory:  Address | Value
--------|-------
1000    |  5
← a lives here
2000    |  1000  ← b (pointer) stores address of a
\`\`\`
...
int *b = &a;        // pointer b stores address of a
int **c = &b;       // double pointer — stores address of pointer b
printf('Value of a = %d\\n', a);               // 5
printf('Address of a = %u\\n', &a);            // 2831685116 (example)
printf('b = address of a = %u\\n', b);         // 2831685116
printf('*b = value at address in b = %d\\n',*b);
// 5
printf('c = address of b = %u\\n', c);         // 2831685120
printf('**c = value via double pointer = %d\\n',**c); // 5
return 0;
}
OUTPUT:
Value of a = 5  Address of a (stored in b) = 2831685116  Value of b = address of a = 2831685116  *b = value at address = 5  c = address of b = 2831685120  **c = value via double pointer = 5
\`\`\`
...
int *ptr = arr;
  // ptr points to arr[0] — address say 1000ptr++;           
// ptr now points to arr[1] — address 1004 (int=4 bytes)ptr+2;
           // points to arr[3] — address 1012*(ptr+1);        
// value at arr[1] = 20// Pointer subtraction gives number of elements between them
int *p1 = &arr[0];  
// address 1000int *p2 = &arr[4];
  // address 1016printf('%d', p2-p1); 
...
float marks;         // float member
  char address[100];   // another character array
};
\`\`\`
...
int main() {
  struct Employee e1;        // create structure variable e1
e1.id = 101;               // access using DOT operator
e1.salary = 55000.0;
...
\`\`\`
struct Employee e1 = {101, 'Bob', 60000.0, 9876543210};
struct Employee *ptr = &e1;
  // pointer to structure
// Two ways to access via pointer:printf('%d', (*ptr).id);
    // method 1: dereference then dotprintf('%d', ptr->id);      
// method 2: ARROW operator → (preferred)printf('%s', ptr->name);
...
// Accessing nested structure members:struct Employee emp;emp.id = 101;strcpy(emp.addr.city, 'Hyderabad');emp.addr.pincode = 500001;
\`\`\`

## Array of Structures

\`\`\`
struct Student students[3];  // array of 3 Student structures// Storing datastudents[0].roll_no = 1;
strcpy(students[0].name, 'Ravi');students[1].roll_no = 2;
strcpy(students[1].name, 'Priya');students[2].roll_no = 3;
strcpy(students[2].name, 'Arjun');
// Displaying all studentsfor(int i=0; i<3; i++)
...
\`\`\`
// Declaration only — garbage values
int arr[5];
// Declaration + initialization
int arr[5] = {10, 20, 30, 40, 50};
// Partial initialization — rest filled with 0int arr[5] = {10, 20
};   // {10, 20, 0, 0, 0
}
// Size inferred from values
int arr[] = {10, 20, 30}; // size automatically = 3// 2D Array declaration
int matrix[3][4];
// 3 rows, 4 columns
...
int n = 5;for(int i = 0; i < n; i++)
  printf('%d ', arr[i]);
// Output: 10 20 30 40 50// Time Complexity: O(n)
\`\`\`

### Insertion — Add Element at Position

\`\`\`
// Insert value 99 at position pos=2 (shift right first)
int arr[10] = {10, 20, 30, 40, 50
};
int n=5, pos=2, val=99;for(int i = n-1; i >= pos; i--)
// shift elements right
  arr[i+1] = arr[i];arr[pos] = val;  // place new valuen++;
...
\`\`\`
// Delete element at position pos=2int arr[5] = {10, 20, 30, 40, 50
};
int n=5, pos=2;for(int i = pos; i < n-1; i++)  
// shift elements left
  arr[i] = arr[i+1];n--;
// Before: {10, 20, 30, 40, 50
}
...
struct Node *next;
     // NEXT part — pointer to next node
};
// Creating a node dynamically
struct Node *head, *ptr;ptr = (struct Node*) malloc(sizeof(struct Node));ptr->data = 10;ptr->next = NULL;head = ptr;
...
// (iii) Insertion after specified node — O(n) to find position
void insertAfter(struct Node* prevNode, int value) {
if(prevNode == NULL) return;
  struct Node* newNode = createNode(value);
newNode->next = prevNode->next;  // new node → old next
...
// (ii) Deletion at END — O(n)
void deleteAtEnd(struct Node **head) {

  if(*head == NULL) return;
  if((*head)->next == NULL) { free(*head); *head=NULL; return; }
struct Node* temp = *head;
  while(temp->next->next != NULL)  // go to second-last
temp = temp->next;
...
struct Node* temp = head;
  int pos = 0;
  while(temp != NULL) {
    if(temp->data == key) return pos;  // found!
temp = temp->next;
    pos++;
  }
  return -1;  // not found
...
\`\`\`
#define MAX_LEVEL 6struct SkipNode {
  int key;
...
struct SkipNode *header; // header node (−∞)
};
\`\`\`
...
\`\`\`
struct SkipNode* search(struct SkipList *list, int key) {
struct SkipNode *curr = list->header;  // Start from highest level, work down
for(int i = list->level-1; i >= 0; i--) {
while(curr->forward[i] && curr->forward[i]->key < key)
curr = curr->forward[i];  // move right at current level    // Drop down to next level when overshoot  
}

  curr = curr->forward[0];  // move to actual node
...
BEGIN
if top = MAX then STACK OVERFLOW, EXIT
top = top + 1
stack[top] = item
...
BEGIN
if top = 0 then STACK UNDERFLOW, EXIT
item = stack[top]
top = top - 1
END  Time Complexity: O(1)
\`\`\`
...
\`\`\`
// Factorial using recursion — uses system stack
int factorial(int n) {

  if(n == 0) return 1;         // base case — stack unwinds here
return n * factorial(n-1);
   // each call pushed onto stack
}
// factorial(4) stack: [4, [3, [2, [1, [0→1]]]]]
// Unwinds: 1→1→2→6→24
\`\`\`
...
if(front == -1) front = 0;  // first element — set front
queue[++rear] = item;
  printf('%d enqueued\\n', item);
}
...
A Binary Tree is a tree where EVERY NODE has at most TWO children — called the LEFT child and the RIGHT child. A node can have 0, 1, or 2 children, but not more than 2.
\`\`\`

\`\`\`
// Binary Tree Node Structure in Cstruct TreeNode {
int data;
...
Use: Copying a tree, prefix expressions, serialize/deserialize tree
\`\`\`
...
Use: Deleting a tree (delete children before parent), postfix expressions
\`\`\`
...
Level 1: [2,6]  
Level 2: [1,3,5,7]
Use: Shortest path in unweighted tree, printing tree level-by-level
\`\`\`

\`\`\`
void levelOrder(struct TreeNode* root) {
if(!root) return;
  struct TreeNode* queue[100]; int front=0, rear=0;
queue[rear++] = root;
  while(front < rear) {
...
\`\`\`
BST Example:  Search 40: 50→left(30<50)→right(40>30)→FOUND ✓ (3 comparisons)
Search 45: 50→30→40→right→NULL (not found) ✗
\`\`\`
...
•  Max-Heap: Parent ≥ Children for every node (root is the MAXIMUM element)
•  Min-Heap: Parent ≤ Children for every node (root is the MINIMUM element)Heaps are usually implemented using ARRAYS (not pointers) because it's a complete binary tree.
\`\`\`

![Max-Heap and Min-Heap with Array Representation](/DSA%20images/dimg17.png)

\`\`\`
// Max-Heap — Heapify Up (after insert)
...
// Max-Heap — Heapify Down (after extract max)
...
\`\`\`
Graph: A-B, A-C, B-D, C-D, D-E  Vertices: A=0, B=1, C=2, D=3, E=4
Space: O(V²)  |
Edge lookup: O(1)  |
...
\`\`\`
// Adjacency Matrix in Cint graph[5][5] = {  {0,1,1,0,0
},  // A: connects to B,C  {1,0,0,1,0
},  // B: connects to A,D  {1,0,0,1,0
},  // C: connects to A,D  {0,1,1,0,1
},  // D: connects to B,C,E  {0,0,0,1,0
},  // E: connects to D
};
\`\`\`
...
Edge lookup: O(degree(v)) — must scan the list
\`\`\`

\`\`\`
// Adjacency List using array of linked lists#include <stdio.h>
#include <stdlib.h>
#define V 5struct AdjNode { int dest; struct AdjNode* next; };
struct AdjList  { struct AdjNode* head; };
struct Graph    { struct AdjList array[V]; };
void addEdge(struct Graph* g, int src, int dest) {
  // Add dest to src's list
  struct AdjNode* n = malloc(sizeof(struct AdjNode));
n->dest = dest; n->next = g->array[src].head;
g->array[src].head = n;  // For undirected: also add src to dest's list
n = malloc(sizeof(struct AdjNode));
  n->dest = src; n->next = g->array[dest].head;
g->array[dest].head = n;
}
\`\`\`
...
\`\`\`
BFS Concept
BFS starts from a source vertex and explores ALL NEIGHBOURS at the current level before moving to the next level.
...
BFS from A:  
Queue: [A]  → 
Visit A, add neighbours B,C  
Queue: [B,C]→ 
Visit B, add D,E; 
Visit C (no new unvisited)  
Queue: [D,E]→ 
Visit D; 
Visit E  BFS Order: A → B → C → D → E  
Level 0: A  |  
Level 1: B,C  |  
Level 2: D,E
\`\`\`
...
#include <stdio.h>
#include <stdbool.h>
#define V 6int graph[V][V] = {
  // Adjacency Matrix  {0,1,1,0,0,0
},  
// A(0): B,C  {1,0,0,1,1,0
},  // B(1): A,D,E  {1,0,0,0,1,0
},  
// C(2): A,E  {0,1,0,0,0,1
},  // D(3): B,F  {0,1,1,0,0,1
},  
// E(4): B,C,F  {0,0,0,1,1,0
},  // F(5): D,E
};
void BFS(int start) {
...
visited[u] = true;
        queue[rear++] = u;  // enqueue neighbour      
}
}  }}
\`\`\`
...
\`\`\`
DFS Concept
DFS starts from a source vertex and goes as DEEP as possible along each branch before BACKTRACKING.It uses a STACK (or recursion, which uses the call stack) to keep track.
DFS does NOT guarantee shortest path but is excellent for detecting cycles, topological sort, and exploring all paths.
\`\`\`

\`\`\`
Same Graph:   DFS from A (alphabetical neighbour order):  
Visit A → go to B (unvisited)  
Visit B → go to D (unvisited)  
Visit D → backtrack (all neighbours visited)  Back at B → go to E  
Visit E → go to C  
Visit C → backtrack all the way  DFS Order: A → B → D → E → C
\`\`\`
...
\`\`\`
// DFS — Recursive (uses system call stack)bool visited[V] = {false
};
void DFS_recursive(int v) {
  ...
  for(int u=0; u<V; u++) {
    if(graph[v][u]==1 && !visited[u])
      DFS_recursive(u);  // recurse into neighbour  
}}
// DFS — Iterative (explicit stack)
void DFS_iterative(int start) {

  bool visited[V] = {false};
  int stack[V]; int top = -1;
  stack[++top] = start;
  while(top >= 0) {
    int v = stack[top--];
    if(!visited[v]) {
      visited[v] = true;
      printf('%d ', v);
      for(int u=V-1; u>=0; u--)  // push in reverse for correct order
if(graph[v][u]==1 && !visited[u])
          stack[++top] = u;
    }
}}
...
\`\`\`
Array: [64, 25, 12, 22, 11]   Search key: 22  
Step 1: Compare 64 with 22 → No  
Step 2: Compare 25 with 22 → No  
Step 3: Compare 12 with 22 → No  
Step 4: Compare 22 with 22 → YES! Found at index 3 ✓
\`\`\`
...
\`\`\`
Key Requirement
Binary Search ONLY works on a SORTED array. If the array is unsorted, sort it first (or use Linear Search).Idea: Compare key with middle element. If equal → found. If key < mid → search left half. If key > mid → search right half. Each comparison HALVES the search space → O(log n)!
\`\`\`
...
\`\`\`
low=0, high=9  
Iteration 1: mid = (0+9)/2 = 4  → arr[4]=16 < 23 → low = 5  
Iteration 2: mid = (5+9)/2 = 7  → arr[7]=42 > 23 → high = 6  
Iteration 3: mid = (5+6)/2 = 5  → arr[5]=23 = 23 → FOUND at index 5! ✓  Only 3 comparisons for 10 elements! (Linear would need 6)
\`\`\`
...
int step = (int)sqrt(n);  // block size = √n
int prev = 0;  // Jump forward in blocks
  while(arr[step < n ? step : n-1] < key) {
prev = step;
    step += (int)sqrt(n);
    if(prev >= n) return -1;  // gone past end  
}  // Linear search in the block [prev...step]
while(arr[prev] < key) {
    prev++;
...
\`\`\`
Position Formula:  pos = low + [ (key - arr[low]) × (high - low) ]
[ ──────────────────────────────── ]
[
(arr[high] - arr[low])
]  If data is uniformly distributed:  
•  Average case: O(log log n)  ← much faster than O(log n)!  
•  Worst case: O(n)
← when data is not uniform
\`\`\`
...
Pass 1: Compare pairs and swap if needed  [64,34] → swap → [34,64,25,12,22,11,90]  [64,25] → swap → [34,25,64,12,22,11,90]  [64,12] → swap → [34,25,12,64,22,11,90]  [64,22] → swap → [34,25,12,22,64,11,90]  [64,11] → swap → [34,25,12,22,11,64,90]  [64,90] → ok
→ [34,25,12,22,11,64,90] ← 90 at correct position  After pass 1: largest element 90 is at correct position  Need n-1 passes to sort n elements
\`\`\`
...
Pass 1: Min of [64,25,12,22,11] = 11 at index 4 → swap with index 0
[11, 25, 12, 22, 64]  
Pass 2: Min of [25,12,22,64] = 12 at index 2 → swap with index 1
[11, 12, 25, 22, 64]  
Pass 3: Min of [25,22,64] = 22 at index 3 → swap with index 2
[11, 12, 22, 25, 64]  
Pass 4: Min of [25,64] = 25 → already in place
...
void selectionSort(int arr[], int n) {
  for(int i=0; i<n-1; i++) {
    int minIdx = i;
    for(int j=i+1; j<n; j++)
      if(arr[j] < arr[minIdx]) minIdx = j;    // Swap minimum with first element of unsorted part
int temp = arr[minIdx]; arr[minIdx] = arr[i]; arr[i] = temp;
  }}
// Time: O(n²) always | Space: O(1) | Not stable// Advantage: Makes minimum number of swaps — n-1 swaps maximum
\`\`\`
...
\`\`\`
Array: [12, 11, 13, 5, 6]  i=1: key=11 → compare with 12, shift 12 right → insert 11 → [11,12,13,5,6]  i=2: key=13 → 13>12, no shift needed
→ insert 13 → [11,12,13,5,6]  i=3: key=5  → shift 13,12,11 right
→ insert 5  → [5,11,12,13,6]  i=4: key=6  → shift 13,12,11 right
...
int j = i - 1;    // Shift elements > key one position right
while(j >= 0 && arr[j] > key) {
      arr[j+1] = arr[j];
      j--;
    }
...
// Time: O(n) best | O(n²) avg/worst | Stable | Best for nearly sorted
\`\`\`
...
\`\`\`
Array: [38, 27, 43, 3, 9, 82, 10]  DIVIDE:  [38,27,43,3]
[9,82,10]  [38,27]  [43,3]
[9,82]  [10]  [38][27] [43][3]   [9][82] [10]  MERGE (conquer):  [27,38]  [3,43]
[9,82]  [10]  [3,27,38,43]
[9,10,82]  [3,9,10,27,38,43,82]  ← SORTED! ✓
\`\`\`
...
\`\`\`
// Partition — returns index of pivot in correct position
int partition(int arr[], int low, int high) {
int pivot = arr[high];  // choose last element as pivot
int i = low - 1;        // i tracks boundary of < pivot region
...
// Time: O(n logn) avg | O(n²) worst (sorted array + bad pivot)// Space: O(log n) | Not stable | Fastest in practice (good cache)
\`\`\`
...
void heapSort(int arr[], int n) {  // Build max-heap
for(int i=n/2-1; i>=0; i--) heapify(arr,n,i);  // Extract elements one by one
for(int i=n-1; i>0; i--) {
...
\`\`\`
// Problem: Two Sum in SORTED array// Find two numbers that add up to targetint* twoSum(int arr[], int n, int target) {
int left=0, right=n-1;
  while(left < right) {
    int sum = arr[left] + arr[right];
    if(sum == target) return (two pointers);
...
// O(n) time, O(1) space// Problem: Check Palindrome
...
int windowSum=0, maxSum=0;  // Calculate sum of first window
for(int i=0;i<k;i++) windowSum += arr[i];
maxSum = windowSum;  // Slide window: add next, remove first
for(int i=k; i<n; i++) {
...
int low=0, high=n-1;
  while(low<=high) {
    int mid=(low+high)/2;
    if(arr[mid]==target) return mid;    // Check which half is sorted
...
void nextGreater(int arr[], int n) {
  ...
  for(int i=0;i<n;i++) {    // Pop elements smaller than arr[i]
...
\`\`\`
🎉 Course Complete!You have now covered all 22 topics of Data Structures & Algorithms — from the very basics of what a data structure is, through arrays, linked lists, stacks, queues, trees, graphs, sorting, and searching, all the way to advanced interview patterns and a complete cheat sheet.
Remember: DSA is a skill built through PRACTICE. Read once, understand deeply, then code from scratch. Aim for at least 150+ problems across all topics.
