...
/* ── Drag handle between code and output panels ── */
function LabResizeHandle() {
  return (
    <PanelResizeHandle className="group relative flex items-center justify-center w-[5px] bg-white/8 hover:bg-primary/50 data-[resize-handle-active]:bg-primary transition-colors cursor-col-resize z-10">
      <div className="flex flex-col gap-[4px] pointer-events-none">
        {[0, 1, 2, 3, 4].map(i => (
          <div
            key={i}
            className="w-[3px] h-[3px] rounded-full bg-white/20 group-hover:bg-primary/80 group-data-[resize-handle-active]:bg-white transition-colors"
          />
        ))}
      </div>
    </PanelResizeHandle>
...
export default function LabPage() {
  ...
  /* panel refs for programmatic expand/collapse */
  ...
  /* on mobile, collapse the inactive panel */
  useEffect(() => {
  ...
  /* restore both panels when resizing from mobile to desktop */
  useEffect(() => {
    const onResize = () => {
      if (window.innerWidth >= 1024) {
        if (codeCollapsed)   codePanelRef.current?.expand()
        if (outputCollapsed) outputPanelRef.current?.expand()
      }
    }
    window.addEventListener("resize", onResize)
    return () => window.removeEventListener("resize", onResize)
  }, [codeCollapsed, outputCollapsed])

  function toggleMaximizeCode() {
    if (outputCollapsed) {
      outputPanelRef.current?.expand()
    } else {
      outputPanelRef.current?.collapse()
            ...
            <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
              ...
              <div className="flex items-center justify-between px-3 py-1.5 bg-[#161b22] border-b border-white/8 shrink-0">
                ...
                <div className="flex items-center gap-0.5">
                  <button
                    onClick={() => navigator.clipboard.writeText(codes[langId]).then(() => toast.success("Copied!"))}
                  ...
                  <button
                    onClick={() => { setCodes(p => ({ ...p, [langId]: lang.starter })); setResult(null); setHtmlSrc(null); toast.info("Reset") }}
                    className="flex items-center gap-1 text-xs text-white/40 hover:text-white/80 px-2 py-1 rounded hover:bg-white/8 transition-colors"
                  >
                  ...
                  <button
                    ...
                    title={outputCollapsed ? "Restore split" : "Maximize editor"}
                  >
                    {outputCollapsed ? <Minimize2 className="h-3.5 w-3.5" /> : <Maximize2 className="h-3.5 w-3.5" />}
                  </button>
                </div>
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#0d1117" }}>
              ...
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-white/8 bg-[#161b22] shrink-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {result && !isHtml && (
                    <button
                      onClick={() => navigator.clipboard.writeText(result.output).then(() => toast.success("Copied!"))}
                  ...
                  <button
                    ...
                    title={codeCollapsed ? "Restore split" : "Maximize output"}
                  >
                    {codeCollapsed ? <Minimize2 className="h-3.5 w-3.5" /> : <Maximize2 className="h-3.5 w-3.5" />}
                  </button>
                </div>
              </div>
