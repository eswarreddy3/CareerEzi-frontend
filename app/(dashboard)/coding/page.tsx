...
export default function CodingPage() {
  ...
  const handleLanguageChange = (newLang: "python" | "java" | "cpp" | "javascript") => {
    setLanguage(newLang)
    ...
    <div className="flex h-[calc(100vh-8rem)] gap-4">
          ...
          <div className="space-y-2">
            {problemsList.map((problem) => (
              ...
              >
                ...
                <Badge 
                  variant="outline" 
                  className={cn("text-xs mt-2", difficultyColors[problem.difficulty])}
                >
                  {problem.difficulty}
                </Badge>
              </button>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-4 min-w-0">
        ...
        <div className="w-full lg:w-3/5 flex flex-col min-h-0">
          {/* Editor Header */}
          <div className="flex items-center justify-between mb-4">
            <Select value={language} onValueChange={(v) => handleLanguageChange(v as typeof language)}>
          ...
          <GlassCard className="flex-1 flex flex-col min-h-0 p-0 overflow-hidden">
            <div className="flex-1 relative">
              <textarea
                ...
                className="absolute inset-0 w-full h-full p-4 bg-transparent font-mono text-sm text-foreground resize-none focus:outline-none leading-relaxed"
                spellCheck={false}
                style={{ 
                  tabSize: 4,
              ...
              <div className="absolute left-0 top-0 bottom-0 w-12 bg-secondary/30 border-r border-border flex flex-col items-end pr-2 pt-4 pointer-events-none">
                {code.split("\n").map((_, index) => (
                  <span key={index} className="text-xs text-muted-foreground leading-relaxed h-[1.5rem]">
                    {index + 1}
                  </span>
                ))}
              </div>
              <div className="absolute inset-0 pl-12">
                <textarea
                  value={code}
                  onChange={(e) => setCode(e.target.value)}
                  className="w-full h-full p-4 bg-transparent font-mono text-sm text-foreground resize-none focus:outline-none leading-relaxed"
                  spellCheck={false}
                  style={{ 
                    tabSize: 4,
                    fontFamily: "var(--font-jetbrains), monospace" 
          ...
          {/* Test Cases Panel */}
          <GlassCard className="mt-4 max-h-48 overflow-y-auto">
