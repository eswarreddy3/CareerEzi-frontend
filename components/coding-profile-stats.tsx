...
import {
  Github, Code2, ExternalLink, Star, Users, FolderGit2, Trophy, Target,
} from "lucide-react"
 ...
 */
export function CodingProfileStats({ withHeading = false }: { withHeading?: boolean }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {connected.map((p) =>
          p.platform === "github" ? <GithubStatsCard key={p.platform} profile={p} />
          : p.platform === "leetcode" ? <LeetcodeStatsCard key={p.platform} profile={p} />
          : null,
        )}
      </div>
    </div>
